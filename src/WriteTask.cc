...
void WriteTask::cpu_run(const Task* task,
  ...
  DoubleAccessor zone_energy_density = regions[0].get_field_accessor(FID_ZE)
      .typeify<double>();
  DoubleAccessor zone_pressure = regions[0].get_field_accessor(FID_ZP)
      .typeify<double>();
  DoubleAccessor zone_rho = regions[0].get_field_accessor(FID_ZR)
      .typeify<double>();
  ...
  if (args.direct_input_params.write_xy_file) {
    ...
    wxy_->write(args.probname, zone_rho, zone_energy_density, zone_pressure,
      zr_itr, ze_itr, zp_itr);
    delete wxy_;
  }
