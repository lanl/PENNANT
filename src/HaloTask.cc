...
void HaloTask::cpu_run(const Task* task,
  ...
  LogicalStructured sides(ctx, runtime, regions[0], "halo sides LogStruct");
  const int* map_crn2crn_next = sides.getRawPtr<int>(FID_MAP_CRN2CRN_NEXT);

  assert(task->regions[1].privilege_fields.size() == 2);
  LogicalStructured points(ctx, runtime, regions[1], "halo points LogStruct");
  ...
  DoubleSOAAccessor pt_weighted_mass = local_points_by_gid
      .getRegionSOAAccessor<double>(FID_PMASWT);
  Double2SOAAccessor pt_force = local_points_by_gid
