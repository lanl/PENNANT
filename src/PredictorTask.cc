...
PredictorTask::PredictorTask(LogicalRegion mesh_zones, LogicalRegion mesh_sides,
  ...
  add_region_requirement(
    RegionRequirement(hydro_sides_and_corners, READ_WRITE, EXCLUSIVE,
  ...
  add_region_requirement(
    RegionRequirement(mesh_edges, WRITE_DISCARD, EXCLUSIVE, mesh_edges));
...
void PredictorTask::cpu_run(const Task* task,
  ...
  double* zone_rho_pred = AbstractedMemory::alloc<double>(
    hydro_write_zones.size());

  assert(task->regions[TWO].privilege_fields.size() == 6);
  LogicalStructured write_sides_and_corners(ctx, runtime, regions[TWO]);
  double2* crnr_force_tot = write_sides_and_corners.getRawPtr<double2>(
    FID_CFTOT);
  double2* side_force_visc = write_sides_and_corners.getRawPtr<double2>(
    FID_SFQ);
  double2* side_force_tts = write_sides_and_corners.getRawPtr<double2>(FID_SFT);
  double2* side_force_pres = write_sides_and_corners.getRawPtr<double2>(
    FID_SFP);
  double* crnr_weighted_mass = write_sides_and_corners.getRawPtr<double>(
    FID_CMASWT);

  double* side_area_pred = write_sides_and_corners.getRawPtr<double>(
    FID_S_DBL_TEMP);
  ...
  for (int side_chunk = 0; side_chunk < args.num_side_chunks; ++side_chunk) {
    ...
    LocalMesh::calcCtrs(sfirst, slast, pt_x_pred, map_side2zone, args.num_sides,
      args.num_zones, map_side2pt1, map_side2pt2, map_side2edge, zone_pts_ptr,
      edge_x_pred, zone_x_pred);

    LocalMesh::calcVols(sfirst, slast, pt_x_pred, zone_x_pred, map_side2zone,
    ...
    LocalMesh::calcMedianMeshSurfVecs(sfirst, slast, map_side2zone,
      map_side2edge, edge_x_pred, zone_x_pred, side_surfp);

    LocalMesh::calcEdgeLen(sfirst, slast, map_side2pt1, map_side2pt2,
      map_side2edge, map_side2zone, zone_pts_ptr, pt_x_pred, edge_len);

    LocalMesh::calcCharacteristicLen(sfirst, slast, map_side2zone,
      map_side2edge, zone_pts_ptr, side_area_pred, edge_len, args.num_sides,
      args.num_zones, zone_dl);
    ...
    // 4. compute forces
    PolyGas::calcForce(zone_pressure, side_surfp, side_force_pres, sfirst,
      slast, map_side2zone);

    TTS::calcForce(zone_area_pred, zone_rho_pred, zone_sound_speed,
