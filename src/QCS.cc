...
//  return ts.tv_sec + (1e-9 * ts.tv_nsec) - zero_time;
//}
//
//inline void set_zero_time() {
//  struct timespec ts;
//  clock_gettime(CLOCK_MONOTONIC, &ts);
//  zero_time = ts.tv_sec + (1e-9 * ts.tv_nsec);
//}
//
...
//  set_zero_time();
  ...
  for (int c = cfirst; c < clast; ++c) {
    ...
    int z0 = z - zfirst;
//    cout << "Corner " << c << ", Zone " << z0 << ", Point " << p << endl;
    z0uc[z0] += pu[p];
  }
//  accum1 += current_time();
//  std::cout << "First loop: " << (int) round(100 * accum1) << std::endl;
//  set_zero_time();
