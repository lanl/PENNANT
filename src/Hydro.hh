...
public:
  ...
  static void advPosHalf(const double dt, const int pfirst, const int plast,
      const double2* pt_x0, const double2* pt_vel0, double2* pt_x_pred);

  static void advPosFull(const double dt, const double2* pt_vel0,
      const double2* pt_accel, const double2* pt_x0, double2* pt_vel,
      double2* pt_x, const int pfirst, const int plast);

  static void calcCrnrMass(const int sfirst, const int slast,
  ...
  static void sumCrnrForce(const double2* side_force_pres,
  ...
  static void calcAccel(const ptr_t* pt_local2globalID,
      const Double2SOAAccessor pf, const DoubleSOAAccessor pmass,
      double2* pt_accel, const int pfirst, const int plast);

  static void calcRho(const double* zvol, const double* zm, double* zr,
      const int zfirst, const int zlast);

  static void calcWork(const double dt, const int* map_side2pt1,
      const int* map_side2pt2, const int* map_side2zone,
      const int* zone_pts_ptr, const double2* side_force_pres,
      const double2* side_force_visc, const double2* pt_vel,
      const double2* pt_vel0, const double2* pt_x_pred, double* zone_energy_tot,
      double* zone_work, const int side_first, const int side_last);

  static void calcWorkRate(const double dt, const double* zone_vol,
      const double* zone_vol0, const double* zone_work,
      const double* zone_pressure, double* zone_work_rate, const int zfirst,
      const int zlast);

  static void calcEnergy(const double* zone_energy_tot, const double* zone_mass,
      double* zone_energy_density, const int zfirst, const int zlast);

  static void calcDtCourant(double& dtrec, char* msgdtrec, const int zfirst,
      const int zlast, const double* zdl, const double* zone_dvel,
      const double* zone_sound_speed, const double cfl);

  static void calcDtVolume(const double dtlast, double& dtrec, char* msgdtrec,
      const int zfirst, const int zlast, const double* zvol,
      const double* zvol0, const double cflv);

  static void calcDtHydro(const double dtlast, const int zfirst,
      const int zlast, const double* zone_dl, const double* zone_dvel,
...
private:
  ...
  void initRadialVel(const double vel, const int pfirst, const int plast,
      double2* pt_vel);

  void sumEnergy(const double* zetot, const double* zarea, const double* zvol,
      const double* zm, const double* smf, const double2* px, const double2* pu,
      const int* map_side2pt1, const int* map_side2zone,
      const int* zone_pts_ptr, double& ei, double& ek, const int zfirst,
      const int zlast, const int sfirst, const int slast);
