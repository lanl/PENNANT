...
void GenerateGlobalMesh::colorPartitions(Coloring* zone_map,
  ...
  else if (meshtype == "hex") colorPartitionsHex(zone_map, pt_map);
}

void GenerateGlobalMesh::setupHaloCommunication(int color,
    std::vector<int>* master_colors, std::vector<int>* slave_colors,
  ...
  for (int i = 0; i < master_points.size(); i++) {
    (*halo_pts_map)[color].points.insert(
      pointLocalToGlobalID(master_points[i]));
  }
}
