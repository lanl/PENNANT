...
Hydro::Hydro(const InputParameters& params, LocalMesh* m,
    ...
    Context ctx, Runtime* rt) :
      ...
      ctx(ctx),
      runtime(rt),
      zones(ctx, rt, "hydro zones LogStruct"),
      sides_and_corners(ctx, rt, "hydro sides LogStruct"),
      edges(ctx, rt, "hydro edges LogStruct"),
      points(ctx, rt, "hydro points LogStruct"),
      bcx_chunks(ctx, rt, "bcx chunks LogStruct"),
      bcy_chunks(ctx, rt, "bcy chunks LogStruct"),
      params(params),
...
void Hydro::init() {
  ...
  const int* zone_chunks_CRS = mesh->zone_chunks.getRawPtr<int>(
    FID_ZONE_CHUNKS_CRS);
  const int* pt_chunks_CRS = mesh->point_chunks.getRawPtr<int>(
  ...
  for (int zch = 0; zch < numzch; ++zch) {
    ...
    fill(&zone_energy_density[zfirst], &zone_energy_density[zlast],
      energy_init);
...
#pragma ivdep
      for (int z = zfirst; z < zlast; ++z) {
        if (zx[z].x > (subrgn_xmin - eps) && zx[z].x < (subrgn_xmax + eps)
            && zx[z].y > (subrgn_ymin - eps) && zx[z].y < (subrgn_ymax + eps)) {
...
#pragma ivdep
  ...
  for (int i = 0; i < bcx.size(); ++i) {
    args.boundary_conditions_x.push_back(
      LocalMesh::getXPlane(bcx[i], mesh->num_pts, pt_x));
    std::vector<int> pchb_CRS;
    LocalMesh::getPlaneChunks(args.boundary_conditions_x[i], pt_chunks_CRS,
      numpch, pchb_CRS);
  ...
  std::copy(bcx_point_chunk_CRS_concatenated.begin(),
    bcx_point_chunk_CRS_concatenated.end(), bcx_chunks_CRS);
  bcx_chunks.unMapPRegion();

  count = 0;
  std::vector<int> bcy_point_chunk_CRS_concatenated;
  for (int i = 0; i < bcy.size(); ++i) {
    args.boundary_conditions_y.push_back(
      LocalMesh::getYPlane(bcy[i], mesh->num_pts, pt_x));
    std::vector<int> pchb_CRS;
    LocalMesh::getPlaneChunks(args.boundary_conditions_y[i], pt_chunks_CRS,
      numpch, pchb_CRS);
  ...
  std::copy(bcy_point_chunk_CRS_concatenated.begin(),
    bcy_point_chunk_CRS_concatenated.end(), bcy_chunks_CRS);
...
Future Hydro::doCycle(Future future_step) {
  PredictorPointTask predictor_point_launcher(mesh->points.getLRegion(),
    mesh->point_chunks.getLRegion(), points.getLRegion(), serial.getBitStream(),
  ...
  PredictorTask predictor_launcher(mesh->zones.getLRegion(),
    mesh->sides.getLRegion(), mesh->zone_pts.getLRegion(),
    mesh->points.getLRegion(), mesh->side_chunks.getLRegion(),
    mesh->edges.getLRegion(), zones.getLRegion(),
    sides_and_corners.getLRegion(), points.getLRegion(), serial.getBitStream(),
    serial.getBitStreamSize());
  ...
  CorrectorTask corrector_launcher(mesh->zones.getLRegion(),
    mesh->sides.getLRegion(), mesh->zone_pts.getLRegion(),
    mesh->points.getLRegion(), mesh->edges.getLRegion(),
    mesh->local_points_by_gid.getLRegion(), mesh->point_chunks.getLRegion(),
    mesh->side_chunks.getLRegion(), mesh->zone_chunks.getLRegion(),
    zones.getLRegion(), sides_and_corners.getLRegion(), points.getLRegion(),
    bcx_chunks.getLRegion(), bcy_chunks.getLRegion(), serial.getBitStream(),
    serial.getBitStreamSize());
...
/*static*/
void Hydro::calcCrnrMass(const int sfirst, const int slast,
    const double* zone_area_pred, const double* side_mass_frac,
    const int* map_side2zone, const int* zone_pts_ptr,
    const double* zone_rho_pred, double* crnr_weighted_mass) {
#pragma ivdep
  for (int s = sfirst; s < slast; ++s) {
    int s3 = LocalMesh::mapSideToSidePrev(s, map_side2zone, zone_pts_ptr);
    int z = map_side2zone[s];

    double m = zone_rho_pred[z] * zone_area_pred[z] * 0.5
               * (side_mass_frac[s] + side_mass_frac[s3]);
...
/*static*/
void Hydro::calcAccel(const ptr_t* pt_local2globalID,
    const Double2SOAAccessor pf, const DoubleSOAAccessor pmass,
    double2* pt_accel, const int pfirst, const int plast) {
...
void Hydro::sumEnergy(const double*__restrict__ zetot,
  ...
  for (int s = sfirst; s < slast; ++s) {
    ...
    double cvol = zarea[z] * px[p1].x * 0.5
                  * (side_mass_frac[s] + side_mass_frac[s3]);
...
/*static*/
void Hydro::calcDtVolume(const double dtlast, double& dtrec, char* msgdtrec,
    const int zfirst, const int zlast, const double* zvol, const double* zvol0,
    const double cflv) {
...
void Hydro::calcDtHydro(const double dtlast, const int zfirst, const int zlast,
  ...
  calcDtCourant(dtchunk, msgdtchunk, zfirst, zlast, zone_dl, zone_dvel,
    zone_sound_speed, cfl);
...
void Hydro::writeEnergyCheck() {
  ...
  const int* side_chunks_CRS = mesh->side_chunks.getRawPtr<int>(
    FID_SIDE_CHUNKS_CRS);
  ...
  for (int sch = 0; sch < mesh->num_side_chunks; ++sch) {
    ...
    int zfirst = LocalMesh::side_zone_chunks_first(sch, map_side2zone,
      side_chunks_CRS);
    int zlast = LocalMesh::side_zone_chunks_last(sch, map_side2zone,
      side_chunks_CRS);
  ...
  if (my_color == 0) {
    ...
    cout << "Energy check:  " << "total energy  = " << setw(14) << ei + ek
         << endl;
...
void Hydro::copyZonesToLegion(LogicalUnstructured& global_zones) {
  ...
  DoubleSOAAccessor energy_density_acc = global_zones
      .getRegionSOAAccessor<double>(FID_ZE);
  DoubleSOAAccessor pressure_acc = global_zones.getRegionSOAAccessor<double>(
    FID_ZP);
