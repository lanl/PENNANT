...
CorrectorTask::CorrectorTask(LogicalRegion mesh_zones, LogicalRegion mesh_sides,
    LogicalRegion mesh_zone_pts, LogicalRegion mesh_points,
    LogicalRegion mesh_edges, LogicalRegion mesh_local_points,
  ...
  add_field(0, FID_ZDL);
  add_field(0, FID_ZVOL0);
  add_region_requirement(
    RegionRequirement(hydro_zones, READ_ONLY, EXCLUSIVE, hydro_zones));
  add_field(1, FID_ZDU);
  add_field(1, FID_ZSS);
  add_field(1, FID_ZM);
  add_field(1, FID_ZP);
  add_region_requirement(
    RegionRequirement(hydro_zones, READ_WRITE, EXCLUSIVE, hydro_zones));
  add_field(2, FID_ZR);
  add_field(2, FID_ZE);
  add_field(2, FID_ZWR);
  add_field(2, FID_ZETOT);
  add_region_requirement(
    RegionRequirement(mesh_sides, READ_ONLY, EXCLUSIVE, mesh_sides));
  add_field(3, FID_SMAP_SIDE_TO_ZONE);
  add_field(3, FID_SMAP_SIDE_TO_PT1);
  add_field(3, FID_SMAP_SIDE_TO_PT2);
  add_field(3, FID_SMAP_SIDE_TO_EDGE);
  add_region_requirement(
    RegionRequirement(mesh_zone_pts, READ_ONLY, EXCLUSIVE, mesh_zone_pts));
  add_field(4, FID_ZONE_PTS_PTR);
  add_region_requirement(
    RegionRequirement(hydro_sides_and_corners, READ_ONLY, EXCLUSIVE,
      hydro_sides_and_corners));
  add_field(5, FID_SFP);
  add_field(5, FID_SFQ);
  add_region_requirement(
    RegionRequirement(mesh_local_points, READ_WRITE, EXCLUSIVE,
      mesh_local_points));
  add_field(6, FID_PF);
  add_region_requirement(
    RegionRequirement(mesh_points, READ_ONLY, EXCLUSIVE, mesh_points));
  add_field(7, FID_PXP);
  add_field(7, FID_PX0);
  add_field(7, FID_PT_LOCAL2GLOBAL);
  add_region_requirement(
    RegionRequirement(mesh_edges, READ_WRITE, EXCLUSIVE, mesh_edges));
  add_field(8, FID_EX);
  add_region_requirement(
    RegionRequirement(mesh_zones, READ_WRITE, EXCLUSIVE, mesh_zones));
  add_field(9, FID_ZX);
  add_field(9, FID_ZVOL);
  add_field(9, FID_ZAREA);
  add_region_requirement(
    RegionRequirement(mesh_sides, READ_WRITE, EXCLUSIVE, mesh_sides));
  add_field(10, FID_SAREA);
  add_field(10, FID_SVOL);
  add_region_requirement(
    RegionRequirement(hydro_points, READ_WRITE, EXCLUSIVE, hydro_points));
  add_field(11, FID_PU);
  add_field(11, FID_PU0);
  add_region_requirement(
    RegionRequirement(mesh_points, READ_WRITE, EXCLUSIVE, mesh_points));
  add_field(12, FID_PX);
  add_region_requirement(
    RegionRequirement(mesh_local_points, READ_ONLY, EXCLUSIVE,
      mesh_local_points));
  add_field(13, FID_PMASWT);
  add_region_requirement(
    RegionRequirement(point_chunks, READ_ONLY, EXCLUSIVE, point_chunks));
  add_field(14, FID_POINT_CHUNKS_CRS);
  add_region_requirement(
    RegionRequirement(side_chunks, READ_ONLY, EXCLUSIVE, side_chunks));
  add_field(15, FID_SIDE_CHUNKS_CRS);
  add_region_requirement(
    RegionRequirement(zone_chunks, READ_ONLY, EXCLUSIVE, zone_chunks));
  add_field(16, FID_ZONE_CHUNKS_CRS);
  add_region_requirement(
    RegionRequirement(bcx_chunks, READ_ONLY, EXCLUSIVE, bcx_chunks));
  add_field(17, FID_BCX_CHUNKS_CRS);
  add_region_requirement(
    RegionRequirement(bcy_chunks, READ_ONLY, EXCLUSIVE, bcy_chunks));
...
TimeStep CorrectorTask::cpu_run(const Task* task,
  ...
  Double2SOAAccessor point_force = local_write_points_by_gid
      .getRegionSOAAccessor<double2>(FID_PF);

  assert(task->regions[2].privilege_fields.size() == 4);
  LogicalStructured hydro_write_zones(ctx, runtime, regions[2],
      "hydro write zones LogStruct");
  ...
  for (int pt_chunk = 0; pt_chunk < args.num_point_chunks; ++pt_chunk) {
    ...
    for (int x = 0; x < args.boundary_conditions_x.size(); ++x) {
      int bfirst =
          bcx_chunks_CRS[args.bcx_point_chunk_CRS_offsets[x] + pt_chunk];
      int blast = bcx_chunks_CRS[args.bcx_point_chunk_CRS_offsets[x] + pt_chunk
                                 + 1];
      HydroBC::applyFixedBC(pt_local2globalID, vfixx,
        args.boundary_conditions_x[x], pt_vel0, point_force, bfirst, blast);
    }
    for (int y = 0; y < args.boundary_conditions_y.size(); ++y) {
      int bfirst =
          bcy_chunks_CRS[args.bcy_point_chunk_CRS_offsets[y] + pt_chunk];
      int blast = bcy_chunks_CRS[args.bcy_point_chunk_CRS_offsets[y] + pt_chunk
                                 + 1];
      HydroBC::applyFixedBC(pt_local2globalID, vfixy,
        args.boundary_conditions_y[y], pt_vel0, point_force, bfirst, blast);
    }

    // 5. compute accelerations
    Hydro::calcAccel(pt_local2globalID, point_force, point_mass, pt_accel,
      pfirst, plast);

    // ===== Corrector step =====
    // 6. advance mesh to end of time step
    Hydro::advPosFull(time_step.dt, pt_vel0, pt_accel, pt_x0, pt_vel, pt_x,
      pfirst, plast);
  }
  ...
  for (int side_chunk = 0; side_chunk < args.num_side_chunks; ++side_chunk) {
    ...
    int zfirst = LocalMesh::side_zone_chunks_first(side_chunk, map_side2zone,
      side_chunks_CRS);
    int zlast = LocalMesh::side_zone_chunks_last(side_chunk, map_side2zone,
      side_chunks_CRS);

    // 6a. compute new mesh geometry
    LocalMesh::calcCtrs(sfirst, slast, pt_x, map_side2zone, args.num_sides,
      args.num_zones, map_side2pt1, map_side2pt2, map_side2edge, zone_pts_ptr,
      edge_x, zone_x);
    LocalMesh::calcVols(sfirst, slast, pt_x, zone_x, map_side2zone,
      args.num_sides, args.num_zones, map_side2pt1, map_side2pt2, zone_pts_ptr,
      side_area, side_vol, zone_area, zone_vol);

    // 7. compute work
    std::fill(&zone_work[zfirst], &zone_work[zlast], 0.);
    Hydro::calcWork(time_step.dt, map_side2pt1, map_side2pt2, map_side2zone,
      zone_pts_ptr, side_force_pres, side_force_visc, pt_vel, pt_vel0,
      pt_x_pred, zone_energy_tot, zone_work, sfirst, slast);
  }  // side chunk
  ...
  for (int zone_chunk = 0; zone_chunk < args.num_zone_chunks; ++zone_chunk) {
    ...
    Hydro::calcWorkRate(time_step.dt, zone_vol, zone_vol0, zone_work,
      zone_pressure, zone_work_rate, zone_first, zone_last);

    // 8. update state variables
    Hydro::calcEnergy(zone_energy_tot, zone_mass, zone_energy_density,
      zone_first, zone_last);
    Hydro::calcRho(zone_vol, zone_mass, zone_rho, zone_first, zone_last);

    // 9.  compute timestep for next cycle
    Hydro::calcDtHydro(time_step.dt, zone_first, zone_last, zone_dl, zone_dvel,
      zone_sound_speed, args.cfl, zone_vol, zone_vol0, args.cflv, recommend);
  }
