...
RunStat DriverTask::cpu_run(const Task* task,
  ...
  Driver drv(params, args.add_reduction, args.add_int64_reduction,
    args.min_reduction, args.pbarrier_as_master, args.masters_pbarriers,
    regions[0], points, halos_points, pregions_halos, ctx, runtime);

  RunStat value = drv.run();
  return value;
}

Driver::Driver(const InputParameters& params, DynamicCollective add_reduct,
...
RunStat Driver::run() {
  ...
  while (run_stat.cycle < cstop && run_stat.time < tstop) {
    ...
    if (my_color == 0 && (run_stat.cycle == 1 || run_stat.cycle % dtreport == 0)) {
      ...
      cout << "End cycle " << setw(6) << run_stat.cycle << ", time = "
           << setw(11) << run_stat.time << ", dt = " << setw(11) << time_step.dt
  ...
  if (my_color == 0) {
    ...
    cout << "cycle = " << setw(6) << run_stat.cycle << ",         cstop = "
         << setw(6) << cstop << endl;
    cout << "time  = " << setw(14) << run_stat.time << ", tstop = " << setw(14)
         << tstop << endl;
