...
void Parallel::run(InputParameters input_params, Context ctx,
  ...
  for (std::set<Processor>::const_iterator it = all_procs.begin();
      it != all_procs.end(); it++)
    if ((*it).kind() == Processor::LOC_PROC) num_loc_procs++;

  if (num_loc_procs < num_subregions_) {
    printf(
      "FATAL ERROR: This test uses a must epoch launcher, which requires\n");
    printf(
      "  a separate Realm processor for each subregion.  %d of the necessary\n",
      num_loc_procs);
  ...
  DynamicCollective add_reduction = runtime->create_dynamic_collective(ctx,
    num_subregions_, AddReductionOp::redop_id, &zero, sizeof(zero));

  int64_t int_zero = 0;
  DynamicCollective add_int64_reduction = runtime->create_dynamic_collective(
    ctx, num_subregions_, AddInt64ReductionOp::redop_id, &int_zero,
    sizeof(int_zero));

  TimeStep max;
  DynamicCollective min_reduction = runtime->create_dynamic_collective(ctx,
  ...
  for (int color = 0; color < num_subregions_; color++) {
    ...
    for (int i = 0; i < global_mesh.masters[color].size(); i++) {
      lregions_halos.push_back(
        global_mesh.halos_points[(global_mesh.masters[color])[i]].getLRegion());
      args[color].masters_pbarriers.push_back(
...
void SPMDArgsSerializer::archive(SPMDArgs* spmd_args) {
  ...
  bit_stream_size =
      3 * sizeof(DynamicCollective) + sizeof(DirectInputParameters)
      + sizeof(PhaseBarrier)
      + 5 * sizeof(size_t)
...
void DoCycleTasksArgsSerializer::archive(DoCycleTasksArgs* docycle_args) {
  ...
  bit_stream_size = sizeof(DynamicCollective)
      + 8 * sizeof(double) + 11 * sizeof(int) + sizeof(size_t)
      + (docycle_args->meshtype.length() + 1) * sizeof(char) + sizeof(size_t)
      + docycle_args->boundary_conditions_x.size() * sizeof(size_t)
      + sizeof(size_t)
      + docycle_args->boundary_conditions_y.size() * sizeof(size_t)
      + sizeof(size_t)
      + docycle_args->bcx_point_chunk_CRS_offsets.size() * sizeof(int)
      + sizeof(size_t)
      + docycle_args->bcy_point_chunk_CRS_offsets.size() * sizeof(int);
  for (size_t i = 0; i < docycle_args->boundary_conditions_x.size(); i++)
    bit_stream_size += docycle_args->boundary_conditions_x[i].size()
        * sizeof(int);
  for (size_t i = 0; i < docycle_args->boundary_conditions_y.size(); i++)
    bit_stream_size += docycle_args->boundary_conditions_y[i].size()
        * sizeof(int);
  bit_stream = malloc(bit_stream_size);
  free_bit_stream = true;
