...
LocalMesh::LocalMesh(const InputParameters& params, IndexSpace points,
...
#pragma omp parallel for schedule(static)
  ...
  for (int sch = 0; sch < num_side_chunks; ++sch) {
    ...
    calcCtrs(sfirst, slast, pt_x, map_side2zone, num_sides, num_zones,
      map_side2pt1, map_side2pt2, map_side2edge, zone_pts_ptr, edge_x, zone_x);
    calcVols(sfirst, slast, pt_x, zone_x, map_side2zone, num_sides, num_zones,
      map_side2pt1, map_side2pt2, zone_pts_ptr, side_area, side_vol, zone_area,
      zone_vol);
    calcSideMassFracs(sch, map_side2zone, side_area, zone_area, side_chunks_CRS,
      side_mass_frac);
  }
...
void LocalMesh::populateChunks(const int*__restrict__ map_side2zone,
    int**__restrict__ point_chunks_CRS, int**__restrict__ side_chunks_CRS,
    int**__restrict__ zone_chunks_CRS) {
  ...
  *side_chunks_CRS = side_chunks.getRawPtr<int>(FID_SIDE_CHUNKS_CRS);
  std::copy(side_chunks_vec.begin(), side_chunks_vec.end(),
    &(*side_chunks_CRS)[0]);
  ...
  *point_chunks_CRS = point_chunks.getRawPtr<int>(FID_POINT_CHUNKS_CRS);
  std::copy(pt_chunks_vec.begin(), pt_chunks_vec.end(),
    &(*point_chunks_CRS)[0]);
  ...
  *zone_chunks_CRS = zone_chunks.getRawPtr<int>(FID_ZONE_CHUNKS_CRS);
  std::copy(zone_chunks_vec.begin(), zone_chunks_vec.end(),
    &(*zone_chunks_CRS)[0]);
}
...
void LocalMesh::writeMeshStats() {
  ...
  Future future_sum = Parallel::globalSumInt64(gnump, add_reduction, runtime,
    ctx);
...
/*static*/
void LocalMesh::getPlaneChunks(const std::vector<int>& mapbp,
    const int* pt_chunks_CRS, const int num_pt_chunks, vector<int>& pchb_CRS) {
  pchb_CRS.resize(0);
  ...
  for (int pch = 0; pch < num_pt_chunks; ++pch) {
    ...
    bl = lower_bound(&mapbp[pchb_CRS.back()], &mapbp[mapbp.size()], pl)
        - &mapbp[0];
    pchb_CRS.push_back(bl);
  }
}
...
/*static*/
void LocalMesh::calcCtrs(const int sfirst, const int slast,
    const double2*__restrict__ px, const int*__restrict__ map_side2zone,
    const int num_sides, const int num_zones,
    const int*__restrict__ map_side2pt1, const int*__restrict__ map_side2pt2,
    const int*__restrict__ map_side2edge, const int*__restrict__ zone_pts_ptr,
    double2*__restrict__ ex, double2*__restrict__ zx) {
...
/*static*/
void LocalMesh::calcVols(const int sfirst, const int slast,
    const double2*__restrict__ px, const double2*__restrict__ zx,
    const int*__restrict__ map_side2zone, const int num_sides,
    const int num_zones, const int*__restrict__ map_side2pt1,
    const int*__restrict__ map_side2pt2, const int*__restrict__ zone_pts_ptr,
    double*__restrict__ sarea, double*__restrict__ svol,
    double*__restrict__ zarea, double*__restrict__ zvol) {
  int zfirst = map_side2zone[sfirst];
  int zlast = (slast < num_sides ? map_side2zone[slast] : num_zones);
  ...
  for (int s = sfirst; s < slast; ++s) {
    ...
    if (sv <= 0.) {
      ...
      cerr << "px[p1] = {" << px[p1].x << "," << px[p1].y << "}" << endl;
      cerr << "px[p2] = {" << px[p2].x << "," << px[p2].y << "}" << endl;
      cerr << "zx[z] = {" << zx[z].x << "," << zx[z].y << "}" << endl;
      cerr << "sa[" << s << "] = " << sa << endl;
      count++;
    }
#endif
...
void LocalMesh::calcSideMassFracs(const int side_chunk,
    const int*__restrict__ map_side2zone, const double*__restrict__ side_area,
    const double*__restrict__ zone_area, const int*__restrict__ side_chunks_CRS,
    double*__restrict__ side_mass_frac) {
...
/*static*/
void LocalMesh::calcMedianMeshSurfVecs(const int sfirst, const int slast,
    const int*__restrict__ map_side2zone, const int*__restrict__ map_side2edge,
    const double2*__restrict__ edge_x_pred,
    const double2*__restrict__ zone_x_pred, double2*__restrict__ side_surfp) {
#pragma ivdep
  for (int s = sfirst; s < slast; ++s) {
    int z = map_side2zone[s];
    int e = map_side2edge[s];
...
/*static*/
void LocalMesh::calcEdgeLen(const int sfirst, const int slast,
    const int*__restrict__ map_side2pt1, const int*__restrict__ map_side2pt2,
    const int*__restrict__ map_side2edge, const int*__restrict__ map_side2zone,
    const int*__restrict__ zone_pts_ptr, const double2*__restrict__ pt_x_pred,
    double*__restrict__ edge_len) {
  for (int s = sfirst; s < slast; ++s) {
...
/*static*/
void LocalMesh::calcCharacteristicLen(const int sfirst, const int slast,
    const int*__restrict__ map_side2zone, const int*__restrict__ map_side2edge,
    const int*__restrict__ zone_pts_ptr,
    const double*__restrict__ side_area_pred,
    const double*__restrict__ edge_len, const int num_sides,
    const int num_zones, double*__restrict__ zone_dl) {
  int zfirst = map_side2zone[sfirst];
  int zlast = (slast < num_sides ? map_side2zone[slast] : num_zones);
...
void LocalMesh::sumOnProc(const double*__restrict__ corner_mass,
    const double2*__restrict__ corner_force,
    const int*__restrict__ pt_chunks_CRS, const int num_pt_chunks,
    const int*__restrict__ map_pt2crn_first,
    const int*__restrict__ map_crn2crn_next,
    const ptr_t*__restrict__ pt_local2global,
    DoubleSOAAccessor pt_weighted_mass, Double2SOAAccessor pt_force) {
  for (int point_chunk = 0; point_chunk < num_pt_chunks; ++point_chunk) {
    ...
    for (int point = pfirst; point < plast; ++point) {
      ...
      for (int corner = map_pt2crn_first[point]; corner >= 0; corner =
          map_crn2crn_next[corner]) {
...
void LocalMesh::initParallel(const ptr_t* pt_local2global) {
  ...
  generate_mesh->generateHaloPoints(master_colors, slaved_points_counts,
    slaved_points, slave_colors, master_points_counts, master_points);
...
void LocalMesh::sumCornersToPoints(LogicalStructured& sides_and_corners,
  ...
  HaloTask halo_launcher(sides.getLRegion(), points.getLRegion(),
    local_points_by_gid.getLRegion(), point_chunks.getLRegion(),
    sides_and_corners.getLRegion(), serial.getBitStream(),
    serial.getBitStreamSize());
  ...
  if (slave_colors.size() > 0) {
    // phase 1 as master: master copies partial result in; slaves may not access data
    {
      ...
      copy_launcher.add_copy_requirements(
        RegionRequirement(local_points_by_gid.getLRegion(), READ_ONLY,
          EXCLUSIVE, local_points_by_gid.getLRegion()),
        RegionRequirement(halos_points[0].getLRegion(), READ_WRITE,
          SIMULTANEOUS, halos_points[0].getLRegion()));
  ...
  for (int master = 0; master < master_colors.size(); master++) {
      ...
      masters_pbarriers[master]);  // 3 * cycle + 1
    {
      ...
      copy_launcher.add_copy_requirements(
        RegionRequirement(local_points_by_gid.getLRegion(), READ_ONLY,
          EXCLUSIVE, local_points_by_gid.getLRegion()),
        RegionRequirement(slaved_halo_points[master].getLRegion(),
          AddReductionOp::redop_id, SIMULTANEOUS,
    ...
    }
    {
      ...
      copy_launcher.add_copy_requirements(
        RegionRequirement(local_points_by_gid.getLRegion(), READ_ONLY,
          EXCLUSIVE, local_points_by_gid.getLRegion()),
        RegionRequirement(slaved_halo_points[master].getLRegion(),
          Add2ReductionOp::redop_id, SIMULTANEOUS,
          halos_points[1 + master].getLRegion()));
      copy_launcher.add_src_field(0, FID_PF);
      copy_launcher.add_dst_field(0, FID_GHOST_PF);
      copy_launcher.add_wait_barrier(masters_pbarriers[master]);  // 3 * cycle + 1
    ...
    masters_pbarriers[master] = runtime->advance_phase_barrier(ctx,
      masters_pbarriers[master]);  // 3 * cycle + 2
  }
  ...
  if (slave_colors.size() > 0) {
    // phase 3 as master: everybody can read accumulation
    {
      CopyLauncher copy_launcher;
      copy_launcher.add_copy_requirements(
        RegionRequirement(halos_points[0].getLRegion(), READ_ONLY, EXCLUSIVE,
          halos_points[0].getLRegion()),
        RegionRequirement(local_halos_points[0].getLRegion(), READ_WRITE,
          EXCLUSIVE, local_points_by_gid.getLRegion()));
  ...
  for (int master = 0; master < master_colors.size(); master++) {
    // phase 3 as slave: everybody can read accumulation
    {
      ...
      copy_launcher.add_copy_requirements(
        ...
        RegionRequirement(local_halos_points[1 + master].getLRegion(),
          READ_WRITE, EXCLUSIVE, local_points_by_gid.getLRegion()));
      copy_launcher.add_dst_field(0, FID_PMASWT);
      copy_launcher.add_src_field(0, FID_GHOST_PMASWT);
