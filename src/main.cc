...
InputParameters parseInputFile(InputFile *inp) {
  ...
  if (value.meshtype != "pie" && value.meshtype != "rect"
      && value.meshtype != "hex") {
    cerr << "Error:  invalid meshtype " << value.meshtype << endl;
    exit(1);
  }
  vector<double> params = inp->getDoubleList("meshparams", vector<double>());
  if (params.empty()) {
  ...
  direct_values.nzones_y = (
      params.size() >= 2 ? params[1] : direct_values.nzones_x);
  if (value.meshtype != "pie")
    direct_values.len_x = (params.size() >= 3 ? params[2] : 1.0);
  else
    // convention:  x = theta, y = r
    direct_values.len_x = (params.size() >= 3 ? params[2] : 90.0) * M_PI
        / 180.0;
  direct_values.len_y = (params.size() >= 4 ? params[3] : 1.0);

  if (direct_values.nzones_x <= 0 || direct_values.nzones_y <= 0
      || direct_values.len_x <= 0. || direct_values.len_y <= 0.) {
