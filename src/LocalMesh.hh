...
class LocalMesh {
public:
  LocalMesh(const InputParameters& params, IndexSpace pts,
  ...
  static inline int side_zone_chunks_first(int s, const int* map_side2zone,
      const int* side_chunks_CRS) {
    return map_side2zone[side_chunks_CRS[s]];
  }

  static inline int side_zone_chunks_last(int s, const int* map_side2zone,
      const int* side_chunks_CRS) {
  ...
  static void getPlaneChunks(const std::vector<int>& mapbp,
      const int* pt_chunks_CRS, const int num_pt_chunks,
      std::vector<int>& pchb_CRS);

  // compute edge, zone centers
  static void calcCtrs(const int sfirst, const int slast, const double2* px,
      const int* map_side2zone, const int num_sides, const int num_zones,
      const int* map_side2pt1, const int* map_side2pt2,
      const int* map_side2edge, const int* zone_pts_ptr, double2* ex,
      double2* zx);

  // compute side, corner, zone volumes
  static void calcVols(const int sfirst, const int slast, const double2* px,
  ...
  static void calcMedianMeshSurfVecs(const int sfirst, const int slast,
      const int* map_side2zone, const int* map_side2edge,
      const double2* edge_x_pred, const double2* zone_x_pred,
      double2* side_surfp);

  static void calcEdgeLen(const int sfirst, const int slast,
      const int* map_side2pt1, const int* map_side2pt2,
      const int* map_side2edge, const int* map_side2zone,
      const int* zone_pts_ptr, const double2* pt_x_pred, double* edge_len);

  static void calcCharacteristicLen(const int sfirst, const int slast,
      const int* map_side2zone, const int* map_side2edge,
      const int* zone_pts_ptr, const double* side_area_pred,
      const double* edge_len, const int num_sides, const int num_zones,
      double* zone_dl);

  void sumCornersToPoints(LogicalStructured& sides_and_corners,
      DoCycleTasksArgsSerializer& serial);

  static void sumOnProc(const double* corner_mass, const double2* corner_force,
      const int* pt_chunks_CRS, const int num_pt_chunks,
      const int* map_pt2crn_first, const int* map_crn2crn_next,
      const ptr_t* pt_local2global, DoubleSOAAccessor pt_weighted_mass,
...
private:
  ...
  void initSideMappingArrays(const std::vector<int>& cellstart,
      const std::vector<int>& cellnodes, int* map_side2zone, int* map_side2pt1,
      int* map_side2pt2);

  void initEdgeMappingArrays(const int* map_side2zone, const int* zone_pts_ptr,
      const int* map_side2pt1, const int* map_side2pt2, int* map_side2edge);

  void populateChunks(const int* map_side2zone, int** point_chunks_CRS,
      int** side_chunks_CRS, int** zone_chunks_CRS);

  void populateInverseMap(const int* map_side2pt1, int* map_pt2crn_first,
  ...
  void calcSideMassFracs(const int side_chunk, const int* map_side2zone,
      const double* side_area, const double* zone_area,
